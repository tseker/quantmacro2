function [k1,prodn1,dist, v_out, n_pol,kk] = aiyagari9(r0,w0,lambda0, tau, v)
...
global beta mu s b N prob kap c_pol maxkap nkap minkap
...
% Initialising matrices and distance
    ...
    % Additional variables
     prodwage_gd = w0.*s(st_gd(:,1));
     rkap_gd = (1+r0)*kap(st_gd(:,2));

     % Optimization
        % Golden search bounds
            cmin_gd = 1e-2*ones(1,N*nkap);
            cmax_gd = lambda0*prodwage_gd.^(1-tau) + rkap_gd + b; % BC with a'=-b and n=1
test = 10;
while test > 0.01
        % Policy functions and expected value
            [c_pol, n_pol, kk, tv, indU] = golden9(prodwage_gd,rkap_gd,cmin_gd,cmax_gd,prob_gd*v,lambda0, tau);
        % Recover matrix form
...
% Aggregate labour (productivity included)
    prodn1 = sum(sum(s' .* n_pol .* dist));
