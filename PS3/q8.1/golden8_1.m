function [x1,n1,a1,f1,indU1] = golden8_1(prodwage,rkap,cmin,cmax,expV,lambda)

global beta mu kap phi B tau
...
% Functions
    % Labour supply
    labsup = @(c) (lambda*(1-tau) * prodwage.^(1-tau) .* c.^(-mu) / B) .^ (1/(phi+tau));
    % Utility
    util = @(c,n) (c.^(1-mu))/(1-mu)-B*(n.^(1+phi))/(1+phi);
    % Savings
    aprime = @(c,n) lambda*(prodwage.*n).^(1-tau) + rkap - c;
...
% Bounds on a' (necessary if command to avoid error due to empty vector)
if max(a1) > kap(end)
