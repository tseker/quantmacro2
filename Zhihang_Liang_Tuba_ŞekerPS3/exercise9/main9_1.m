...
tolr            = 1e-5;
tollambda       = 1e-4;
r_t             = (r0_post-r0)/T.*(1:T) + r0 ;      % initial guess for r: sudden jump to new SS
lambda_t        =  (lambda0_post-lambda0)/T.*(1:T) + lambda0 ; % initial guess for lambda
...
while testr > tolr & testlambda> tollambda
    ...
    lambda_t = 0.9* lambda_t + 0.1 * lambda_imp;
    testr   = max(abs(r_t-r_imp));
    r_t = 0.95 * r_t + 0.05 * r_imp;
end

% Value right after the shock
[~,~,~,v_t,lambda_dist] = deterministicseq9(r_t,lambda_t,T, v, v_post, dist, dist_post,kk_post, true);
