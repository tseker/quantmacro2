...
shock   = 0.01;                                                         % initial shock
rho     = 0.9;                                                          % shock persistence

% TFP sequence
aux_t   = [0:(T-1)];                                                    % aux variable for t-1
A_t     = 1  + rho.^aux_t * shock;                          % overall TFP shock; rho.^aux_t : decay of the shock over time; 
...
% Wealth Equivalence Gain.
% v^*(a+delta, epsilon) = v^0(a,epsilon).
% need to take care of the bound.
ind_temp = zeros(N,nkap);
weight_temp = zeros(N,nkap);
wealth_gain = zeros(N,nkap);
wealth_gain_percent = zeros(N,nkap);
for j = 1:N
    for i = 1:nkap
        ind_temp(j,i) = find(v0(j,i)>=v(j,:),1,'last');
        if ind_temp ~=200 
            weight_temp(j,i) = ( v0(j,i)-v(j,ind_temp(j,i)) )./(v(j,ind_temp(j,i)+1)-v(j,ind_temp(j,i)));
            wealth_gain(j,i) = kap(ind_temp(j,i)+1)*weight_temp(j,i) + kap(ind_temp(j,i))*(1-weight_temp(j,i))-kap(i);
            wealth_gain_percent(j,i) = (wealth_gain(j,i) )/abs(kap(i));
        else
            ind_temp(j,i) = find(v(j,i)>=v0(j,:),1,'last');
            weight_temp(j,i) = ( v(j,i)-v0(j,ind_temp(j,i)) )./(v0(j,ind_temp(j,i)+1)-v0(j,ind_temp(j,i)));
            wealth_gain(j,i) = kap(ind_temp(j,i)+1)*weight_temp(j,i) + kap(ind_temp(j,i))*(1-weight_temp(j,i));
            wealth_gain(j,i) = kap(i)-wealth_gain(j,i);
            wealth_gain_percent(j,i) =(wealth_gain(j,i) )./abs(kap(i));
        end
    end
end

wealth_eq_mean_percentage = sum(wealth_gain_percent(:).*dist(:));
wealth_eq_mean = sum(wealth_gain(:).*dist(:));
fprintf("The average wealth equivalent is %3.2f. \n", wealth_eq_mean);

fprintf("The average wealth equivalent in percentage is %3.2f%%.\n",100*wealth_eq_mean_percentage);
...
figure(4) % Consumption equivalents
    plot(kap,[wealth_gain(1,:)' wealth_gain(3,:)' wealth_gain(5,:)'],'LineWidth',lwidth)
    % Options
    grid on
    ylabel('wealth equivalent')
    xlabel('asset holdings')
    set(gca,'FontSize',fsize)
    hleg1 = legend('Lowest Skill Shock', 'Intermediate Skill Shock', 'Highest Skill Shock','Location','northeast');
