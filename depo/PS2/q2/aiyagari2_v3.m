...
while test>1e-8
    ...
    for j=1:N
        kk(j,:) = linInterp(kap,a_imp(j,:),kap);
    end
...
size= N*nkap;
indAnext = spalloc(nkap, size, size); % create a sparse matrix by sizexsize

a_pol=reshape(kk',size,1); % Reshape the matrix kk into a column vector a_pol

% Due to interpolation, some a' are not on the grid. Use weights
% for the nearest elements that belong to the grid.
[auxL,auxU,auxW] = getWeights(a_pol, kap', "cap"); % find the nearest elements 
% (auxL and auxU) and their corresponding weights (auxW) for each element in a_pol

% Assign Weights to indAnext:
indAnext(auxL + ((1:size)' - 1) * nkap) = auxW;
indAnext(auxU + ((1:size)' - 1) * nkap) = (1 - auxW);

% Multiplying by Probabilities and Creating Q
% Create the transition matrix Q by multiplying each column of indAnext by 
% the corresponding probability and stacking the resulting matrices.
...
for j = 1:N
    ...
    auxQ = indAnext .* auxProb;
    Q = [Q; auxQ];
end

            
%% Another: Create Q Matrix by vectorized matrix Q_mat

    Q_mat = spalloc(size,size,2*N*size); % 2 possible assets in next period (interpolation), nZ productivity states
    for zz=1:N
        for aa=1:nkap
            st = find((mat.ea(:,id.z)==zz) & (mat.ea(:,id.a)==aa));
            [auxL, auxU, auxW] = getWeights(a_pol(st),kap,"cap");
            Q_mat(mat.ea(:,id.a)==auxL,st) = prob(zz,:) * auxW;
            Q_mat(mat.ea(:,id.a)==auxU,st) = prob(zz,:) * (1-auxW);
        end
    end
