...
function aggregateK = aiyagari2_v4(r0,tau0,wage)
...
while test>1e-8
    iter= iter+1;
    %Implied consumption:
    c_imp = (beta*(1+r0*(1-tau0))*prob*(c0).^(-mu)).^(1/-mu);

    %Implied savings:
    % Given c_imp, a', and productivity, obtain a by budget constraint
    a_imp = (kap+c_imp-(1-tau0)*wage*s')/(1+r0*(1-tau0));
    ...
    c_egm = (1-tau0)*(wage*s'+r0*kap)+ kap - kk;
