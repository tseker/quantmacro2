...
while (testr>0.001)
    ...
    k1= aiyagari2_v2(r0,optH); %calculating asset supply
...
%% Sort households by wealth, and compute the share of total wealth hold by each wealth quintile.

% Sort households by wealth
[~, idx] = sort(kk(:));

% Compute cumulative distribution of wealth
cumDist = cumsum(dist(idx));

% Identify quintiles
quintiles = discretize(cumDist, [0, 0.2, 0.4, 0.6, 0.8, 1]);

% Compute wealth share by quintile
wealthShares = accumarray(quintiles, kk(idx), [], @sum) / aggregateK;

% Compute mean income by wealth quintile
incomeByWealth = accumarray(quintiles, s(dist(idx)), [], @mean);

% Display results
disp('Wealth Shares by Quintile:');
disp(wealthShares);
disp('Mean Income by Wealth Quintile:');
disp(incomeByWealth);
