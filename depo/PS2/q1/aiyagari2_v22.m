...
%% 
% Vectorize util
% Assuming s is a column vector, wage is a scalar, and kap is a row vector
% If the dimensions of s, wage, and kap are different, you need to ensure compatibility

% Initialize utilm matrix
utilm = zeros(N, nkap, nkap);


% Compute cons for all combinations of s and k
cons_all = s .* wage + (1 + r0) * f(:)';

% Compute util for all combinations of s and k
util_all = (cons_all.^(1-mu) - 1) / (1-mu);

% Identify infeasible choices
infeasible = cons_all <= 0;

% Set infeasible choices to a large negative value
util_all(infeasible) = -10000;

% Reshape util_all to match the dimensions of utilm
utilm = reshape(util_all, N, nkap, nkap);
