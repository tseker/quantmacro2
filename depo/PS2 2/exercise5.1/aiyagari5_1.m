...
function aggregateK = aiyagari5_1(r0)
...
% FOC_n
focn = @(sj,a,aprime,n) B*n^(phi)-(wage*sj*((1+r0)*a+wage*sj*n-aprime)^(-mu));
...
for j=1:N
   for i=1:nkap
       for k=1:nkap
           nlow = (kap(k)-(1+r0)*kap(i))/(wage*s(j))+1e-4 ;
           nup = 1 ;
           testn=1;
           while (testn>0.001 && 1-nlow>1e-3) 
               n0 = 0.5*(nup-nlow)+nlow;
               f=focn(s(j),kap(i),kap(k),n0);
                testn=abs(f)
                if f>0
                   nup=n0;
                else 
...
while test>1e-8
    iter= iter+1;
    %Implied consumption:
    c_imp = (beta*(1+r0)*prob*(c0).^(-mu)).^(1/-mu)-B*(nopt(i,:,j).^(1+phi))/(1+phi);
